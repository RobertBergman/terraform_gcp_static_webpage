...
void DrawSpaceship(const Spaceship* ship) {
    ...
    // Only blink if invulnerable but NOT when shield is active
    if (IsSpaceshipInvulnerable(ship) && !ship->shieldIsActive) {
    ...
    if (ship->shieldIsActive) {
        DrawCircleLines(ship->position.x, ship->position.y, SPACESHIP_SIZE * 2.0f, BLUE);
    }
...
bool IsSpaceshipInvulnerable(const Spaceship* ship) {
    return ship->invulnerableTime > 0 || ship->shieldIsActive;
}

void ActivateShield(Spaceship* ship) {
