...
void DrawGame(const GameState* state) {
    switch (state->state) {
        ...
        case GAME_STATE_PAUSED:
            ...
            snprintf(scoreText, sizeof(scoreText), "Shields: %d", state->ship.shieldCount);
            DrawText(scoreText, 10, 85, 20, WHITE);
            
            for (int i = 0; i < state->ship.lives; i++) {
                Vector2 p1 = {30.0f + i * 25, 115};
                Vector2 p2 = {20.0f + i * 25, 130};
                Vector2 p3 = {40.0f + i * 25, 130};
